...
namespace LudoGame.test
{
    public class UnitTest1
    {
        ...
        [Theory]
        [InlineData(56)]
        public void TestIfUserHasReachedGoal(int count)
        {
            var gameMotor = new GameMotor();
            var user = new User();
            var pawn = new Pawn(1, 28, "Red"){ Count = count };
            gameMotor.CheckIfReachedGoal(user, pawn);

            Assert.True(pawn.HasReachedGoal);
        }
